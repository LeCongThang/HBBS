...
class Database
{
    ...
    private function __construct()
    {
        ...
        if ($this->connection === null) {
            ...
            $settings = parse_ini_file(ROOT_PATH . 'app/config/settings.ini', true);
    ...
    public function deletes($table, array $ids)
    {
        $inQuery = implode(',', array_fill(0, count($ids), '?'));
        $this->statement = $this->connection->prepare('DELETE FROM ' . $table . ' WHERE id in ('.$inQuery.')');
        $index = 1;
        foreach ($ids as $id) {
            $this->bindValue($index, $id);
            $index++;
        }
